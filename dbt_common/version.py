...
def _get_version_from_venv_cache_info() -> str:
    # Assuming the standard structure of our released virtual environment
    # docker image (venv cache) look for the file which contains the release
    # number and use that release number to formulate the reported version.
    try:
        today = date.today()
        major = today.year
        minor = today.month

        # We find the file by relative position, since dbt is sometimes run in
        # production in a chroot environment which makes it difficult to use an
        # consistent absolute path.
        venv_parent_dir = Path(__file__).parent.parent.parent.parent.parent.parent
        venv_cache_info_file = venv_parent_dir.joinpath("venv-cache-info.json")

        with open(venv_cache_info_file) as f:
            info_json = json.load(f)

        patch = int(info_json["release_tag"])

        if patch < 1:
            raise ValueError(f"Invalid release number: {patch}")

        return f"{major}.{minor}.{patch}"
    except Exception:
        # Although the above try block should succeed in production, we can't
        # easily log failures because logging is not initialized when this
        # function iws called, so we fall back on the same behavior we want for
        # local development, which is to report version "2024.0.0dev"
        return "2024.0.0dev"


# Mantle is notionally versionless, but we assign an internal version for
# debugging and tracking. At release-time it will be set to YYYY.MM.R
# where R represents the incrementing GitHub release number.
__version__: str = _get_version_from_venv_cache_info()
installed: semver.VersionSpecifier = get_installed_version()
